...
from home.models import 通讯, 书讯, 书评, 观点, 文艺, 译林, 文史, 论文, 古籍, 书库
...
def dashboard_callback(request, context):
    """
    Dashboard 回调函数
    返回包含最新评论、最新问答和数据统计的 context
    """
    # 获取最新评论（最近10条）
    latest_comments = Comment.objects.select_related('content_type').order_by('-submit_date')[:10]
    
    # 获取最新问题（最近10条）
    latest_questions = Question.objects.select_related('author').order_by('-created_at')[:10]
    
    # 获取最新回答（最近10条）
    latest_answers = Answer.objects.select_related('question', 'author').order_by('-created_at')[:10]
